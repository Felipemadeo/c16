...
function draw() {
 ...
 if(gameState === PLAY){
  ...
  if (greenB.isTouching(arrowGroup)){
    arrowGroup.destroyEach (greenB);
greenB.destroyEach(arrowGroup);
    score = score +3
    
    }
    
    if (pinkB.isTouching (arrowGroup)){
    arrowGroup.destroyEach (pinkB);
    pinkB.destroyEach (arrowGroup);
    score = score +2
    
    }
    if (blueB.isTouching (arrowGroup)){
      arrowGroup.destroyEach (blueB);
      blueB.destroyEach (arrowGroup);
      score =  score +1
    }
    if (redB.isTouching(arrowGroup)){
      arrowGroup.destroyEach (redB);
      redB.destroyEach (arrowGroup);
      score = score +4
